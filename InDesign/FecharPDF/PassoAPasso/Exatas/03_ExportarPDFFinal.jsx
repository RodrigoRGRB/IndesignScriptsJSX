...
//#include "FuncaoExportarPDFvExatas.jsx"
...
function exportaPDF(carimbo,arquivo,myDocument,intervaloPDF,exibeJanela){
	...
	*/
	...
	alert("Selecione a opção '"+nomePdfPreset+"' na próxima janela","Lucas ®",false);
...
// TRAVA BOTOES DA JANELA

win.grupoPredefinicao.visible = false;
win.grupoPaginas.visible = false;

// FIM TRAVA BOTOES DA JANELA
...
if(rodaPrograma){
	...
	try{	
				...
				if(win.grupoPredefinicao.whichInfo.selection.text == "Configurações do documento atual"){
					...
					f = File.saveDialog("Salvar PDF Final", "*.pdf");
					f = verificaPDF(f);
					//alert(f);
					...
					//exportaPDF("PDF Final",f,app.activeDocument,intervaloPDF,true);
					exportaPDF(carimbo,f,app.activeDocument,intervaloPDF,true);
				...
				if(f != null){
					...
					alert("Etapa 3 concluída com sucesso. Verifique o PDF gerado\nOBS: Este processo faz com que o PDF final perca sua numeração original","Fechar PDF - Lucas ®");
				}
				}
