...
function exportaPDF(carimbo,arquivo,myDocument,intervaloPDF,exibeJanela){
	...
	*/
	...
	alert("Selecione a opção 'ScriptPresetTemp_X1aTemp' na próxima janela","Lucas ®",false);
...
//win.grupoPredefinicao.whichInfo.selection.text == "Configurações do documento atual"
...
if(rodaPrograma){
	...
	try{	
				...
				if(win.grupoPredefinicao.whichInfo.selection.text == "Configurações do documento atual"){
					...
					f = File.saveDialog("Salvar PDF temporário", "*.pdf");
					f = verificaPDF(f);
					//alert(f);
					...
					exportaPDF("X1ATemp",f,app.activeDocument,intervaloPDF,true);
				...
				if(f != null){
					...
					alert("Etapa 1 concluída com sucesso. Selecione agora a etapa 2","Fechar PDF - Lucas ®");
				}
				}
