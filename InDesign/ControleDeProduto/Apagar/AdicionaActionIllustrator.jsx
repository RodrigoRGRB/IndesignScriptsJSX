...
function Open(event)
{
    if(app.layoutWindows.length == app.documents.length) 
    {
        ...
        if(documentoAtual.saved)
        {
            ...
            var navegador_url = "http://localhost:5731/WebSite/ControleDeProduto.aspx?abrindoArquivo=true&arquivo=" + arquivo;
...
function Close(event)
{
    if(app.layoutWindows.length == app.documents.length) 
    {
        ...
        {
            var arquivo = documentoAtual.fullName;
            var navegador_url = "http://localhost:5731/WebSite/ControleDeProduto.aspx?abrindoArquivo=false&arquivo=" + arquivo;
            #include "/I/Objetos/Scripts/NavegadorAir/bin-debug/Navegador_AdobeScript.jsx"
        }
    }
}
