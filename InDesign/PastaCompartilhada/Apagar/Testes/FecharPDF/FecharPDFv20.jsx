...
#includepath "\\rede-sj\usuarios\rodrigo.gabriel\Adobe Scripts\FecharPDF"
...
// Create a palette-type window (a modeless or floating dialog),
var myProgressPanel = new Window("palette", "Fechar PDF  - Versão 2.0 - Rodrigo Gabriel", [150, 150, 600, 270]); 
...
win.grupoProva.whichInfo.add ('item', "OE");
for(i=0; i <= 3;i++){
	
	win.grupoProva.whichInfo.add ('item', "P"+i);
	
	}
win.grupoProva.whichInfo.add ('item', "PX");
win.grupoProva.whichInfo.add ('item', "PE");
win.grupoProva.whichInfo.add ('item', "PA");
win.grupoProva.whichInfo.add ('item', "GRAFICA");
...
win.grupoProva.whichInfo.onChange = function () 
	...
	/*
		...
		if(win.grupoProva.whichInfo.selection.text == "GRAFICA" || win.grupoProva.whichInfo.selection.text == "PE" ){
			win.grupoProva.info2.labelPDF.text = "PDF Preset: PROL";
			}
        
		else{
			win.grupoProva.info2.labelPDF.text = "PDF Preset: BAIXA";
			}
...
win.grupoPredefinicao.whichInfo.onChange = function () 
	{
		...
		if(booleanChkBox == "true"){
			win.grupoPredefinicao.grupoPorcentagem.grupoCheckBox.chkBox.value = false;      // Valor anterior   "False"  -   16/10/2012
		}
	else{
		...
		if(win.grupoPredefinicao.whichInfo.selection.text == "Configurações do documento atual"){
				...
				win.grupoPredefinicao.grupoTamanhoPagina.enabled = false;      // Valor anterior   "False"  -   16/10/2012 
				win.grupoPredefinicao.grupoPorcentagem.enabled = false;           // Valor anterior   "False"  -   16/10/2012
				win.grupoPredefinicao.grupoSangria.enabled = false;                   // Valor anterior   "False"  -   16/10/2012
...
win.buttons.okBtn.onClick = function()
	{
		...
		if(win.grupoPredefinicao.grupoTamanhoPagina.info.largura.inputLargura.text == "" || win.grupoPredefinicao.grupoTamanhoPagina.info.largura.inputLargura.text == 0 ){			
			alert("Digite um valor válido para a largura da página","Editorial - Área Restrita",true);
			}
			...
			alert("Digite um valor válido para a altura da página","Editorial - Área Restrita",true);
			}		
		else if(win.grupoPredefinicao.grupoSangria.info.DirEsq.inputDirEsq.text == ""){
			alert("Digite um valor válido para a sangria Direita/Esquerda","Editorial - Área Restrita",true);
			}	
		else if(win.grupoPredefinicao.grupoSangria.info.InfSup.inputInfSup.text == ""){
			alert("Digite um valor para a sangria Inferior/Superior","Editorial - Área Restrita",true);
			}		
			...
			alert("Digite um valor válido para a porcentagem horizontal","Editorial - Área Restrita",true);
			}
			...
			alert("Digite um valor válido para a porcentagem vertical","Editorial - Área Restrita",true);
			}
		else if(chkIntervalo == false){ //Intervalo das páginas
			alert("Intervalo de páginas inválido","Editorial - Área Restrita",true);
			}
		else if(contemIntervaloInvalido == true){ //Intervalo das páginas
			alert("Intervalo contém páginas inexistentes","Editorial - Área Restrita",true);
			}
		else if(sequenciaInvalida == true){ //Intervalo das páginas
			alert("Sequencia inválida.\nInsira as páginas em ordem crescente","Editorial - Área Restrita",true);
			}
		else if(ultimoCaracter == "," || ultimoCaracter == "-"){ //Verifica se último caractere é um número
			alert("Intervalo de páginas inválido.\nO último caractere deve ser uma página existente","Editorial - Área Restrita",true);
			}		
		else if(todosOsCampos.length>1){
			alert("Utilize ponto (.) ao invés de vírgula (,)","Editorial - Área Restrita",true);
			}
		else{
...
if(win.grupoPaginas.paginas.radIntervalo.value == true){
	...
	try{	
				...
				if(win.grupoPredefinicao.whichInfo.selection.text == "Configurações do documento atual"){
					...
					var configuracaoPagina = win.grupoPredefinicao.whichInfo.selection.text;
                    
					criaCarimbo(carimbo,configuracaoPagina,app.activeDocument,intervaloPDF, f);
				...
				if(f != null){
					removerPresets();
					alert("Arquivo exportado com sucesso","PDF - Editora Poliedro");
				}
					...
					try{
						myProgressPanel.close();
                           ...
                           alert("Não foi possível concluir a operação.\nErro: " + e,"PDF - Editora Poliedro");
				}
		}

}
else{
	alert("Nenhum arquivo aberto","- Editorial - Área Restrita",true);
}