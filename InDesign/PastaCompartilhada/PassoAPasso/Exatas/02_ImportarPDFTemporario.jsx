...
//#include "FuncaoPDFPlacervExatas.jsx"
...
function addConfigJanela (myDocument){
			...
			*/
			...
			/*
			alert(documentBleedBottomOffset);
			alert(documentBleedTopOffset);
			alert(documentBleedInsideOrLeftOffset);
			alert(documentBleedOutsideOrRightOffset);
			alert(pageWidth);
			alert(pageHeight);
			*/
...
if(rodaPrograma){
	...
	f = File.openDialog("Abrir PDF Temporário", "*.pdf");
	//f = verificaPDF(f);
	//alert(f);
	
	
	//********** CAPTURA INTERVALO PDF **************
	...
	//Criar um array com as páginas do PDF
	var intervaloPDFTemp = getPDFPageCount(f);
	
	var numeroDePaginas = intervaloPDFTemp;
	
	//Cria um array com todas as páginas configura o page Range
	var arrayPaginas = 1;
	for(z=2;z<=intervaloPDFTemp;z++){
		arrayPaginas += ","+z;
	}
	...
	//********** FIM DA CAPTURA INTERVALO PDF **************


win.grupoPredefinicao.whichInfo.selection.text == "Exatas";
...
//win.show();
	...
	try{	
					...
					//************* EXIBE PROGRESS BAR ******************
					#targetengine "session"
					myProgressPanel.pnl.myProgressBar.maxvalue = intervaloPDFTemp;
					...
					pdfPlacer(originalDocument,myDocument,f,intervaloPDF);
					
					var resultado = confirm("A primeira página do arquivo PDF é par?");
					...
					if(resultado){
						...
						//alert("aqui");
							...
							var mySection = meuDocumento.sections.add(meuDocumento.pages[0]);
							//mySection = meuDocumento.sections.item(meuDocumento.sections.length);
					...
					var i = 2; //Seleção Exatas
					sangriaDirEsq = parseInt(arrayTXT[(i*8)+3]);
					sangriaInfSup = parseInt(arrayTXT[(i*8)+4]);
					distorcerDeAcordoComMargem = false;
					distorcaoHorizontal = parseInt(arrayTXT[(i*8)+6]);
					distorcaoVertical = parseInt(arrayTXT[(i*8)+7]);
					
					//alert(myDocument+"\n"+sangriaInfSup+"\n"+sangriaDirEsq+"\n"+distorcaoHorizontal+"\n"+distorcaoVertical+"\n"+distorcerDeAcordoComMargem);
				...
				if(f != null){
					...
					alert("Etapa 2 concluída com sucesso. Selecione agora a etapa 3","Fechar PDF - Lucas ®");
				}
				}
