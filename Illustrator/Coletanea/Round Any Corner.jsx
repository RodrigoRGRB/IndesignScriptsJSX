...
// 2005.09.18
// 2005.09.18 Fixed several bugs
// 2005.10.04 Fixed several bugs related to open pathes.
//            Now the script rounds only selected corners.
//            For now, added points on straight sides have unreasonable handles.
// 2005-10-08 Now added points on straight sides do not have unreasonable handles.
// 2005-10-11 Fixed the bug that always rounds 1st corner of close pathes.
// 2005-10-13 remove some wastes. rewrite some comments.
// 2005-10-22 improve speed
// 2005-11-01 Fixed the bug that fails to get the length of path in some cases. ###1
// 2005-12-02 Now works on Illustrator10
...
function roundAnyCorner(){
  ...
  var check_circle = true; // set true or false

  // ------------------------------------------------------
  if(ver10) use_foreground_shape_to_set_radius = true;

  var s = getPathItemsInSelection(1); // extract pathItems which pathpoints length is greater than 1
  ...
  if(!use_foreground_shape_to_set_radius){
    rr = prompt("Radius (Point)", rr); // input the radius ( point );
    if(!rr || isNaN(rr) || rr<=0) return;
  }
      ...
      if(arrEq(q[0],q[1]) && arrEq(q[2],q[3])){  // straight side
        ...
        if(skipList[nxi]){
          ...
          if(r<rr){  // when the length of the side is less than rr * 2
            pnts.push([anc1, getPnt(anc1, getRad(ldir1, anc1), r*hanLen), ldir1, ptyp]);
          } else {
            if(!skipList[i]) pnts.push([anc1, anc1, ldir1, ptyp]);
            anc2 = getPnt(q[3], g+Math.PI, r);
            pnts.push([anc2, getPnt(anc2, g, r*hanLen), anc2 , ptyp]);
          }
          redrawFlg = true;
        }
        ...
        if(skipList[nxi]){
          if(skipList[i]){
            ...
            with(p[nxi]) pnts.push([anchor, rightDirection,
                                   adjHan(anchor, leftDirection, 1-t), ptyp]);
            redrawFlg = true;
          }
        } else { // skipList[nxi] = false
          if(r<rr){  // the length of the side is less than rr * 2
            if(skipList[i]){
              if(!op && i==0){ adjRdirAtEnd = t;
              } else { pnts[pnts.length-1][1] = adjHan(q[0],q[1],t);
              }
              pnts.push([anc1, getPnt(anc1, getRad(ldir1, anc1), r*hanLen),
                         defHan(t,q,0), ptyp]);
            } else {
              pnts.push([anc1, getPnt(anc1, getRad(ldir1, anc1), r*hanLen),
                         ldir1, ptyp]);
...
function getPnt(pt, rad, len){
  return [pt[0] + Math.cos(rad) * len, pt[1] + Math.sin(rad) * len];
}
...
function getT4Len(q, len){
  ...
  if(len==0){ return fullLen;
  } else if(len<0){ len += fullLen; if(len<0) return 0;
  } else if(len > fullLen){ return 1;
  }
  var t,d;
  var t0 = 0;  var t1 = 1;
  var gosa = 0.001;
...
function getLength(k, t){
  var h = t/64;  var hh = h*2;
  var fc = function(t,k){
    //  error (possibly accumulation of rounding) correction
    return Math.sqrt(t*(t*(t*(t*k[0]+k[1])+k[2])+k[3])+k[4]) || 0 }; // ###1
...
// is greater than "n"
function getPathItemsInSelection(n){
  if(documents.length<1) return [];
  var s = activeDocument.selection;
  if (!(s instanceof Array) || s.length<1) return [];
  return extractPathItems(s.slice(0), n);
}
// --------------------------------------
// extract PathItems from "arr" (an Array of PageItems -- ex. selection),
// and return an Array of these Items.  If "pp_length_limit" is specified,
// extract PathItems which PathPoints length is greater than the value.
// "parr" is a variable for recursive call. no need to set.
function extractPathItems(arr, pp_length_limit, parr){
  if(!pp_length_limit) pp_length_limit = 0; // pp_length_limit default = 0
  if(!parr) parr = [];
  var tarr = [];
  var pi,i;
  while(arr.length>0){
    ...
    with(pi){
      if(typename == "PathItem"){
        if (pathPoints.length > pp_length_limit
            && !clipping && !guides   // ignore pathes for clipping path and guide
            ) parr.push(pi);

        // search for PathItems in GroupItem, recursively
      } else if (typename == "GroupItem") {
        for(i=0; i<pageItems.length; i++)  tarr.push(pageItems[i]);
        extractPathItems(tarr, pp_length_limit, parr);
        
        // search for pathitems in CompoundPathItem, recursively
        // ( ### cannot pick up PathItems in GroupItem in CompoundPathItem ### )
      } else if (typename == "CompoundPathItem") {
        for(i=0; i<pi.pathItems.length; i++)  tarr.push(pathItems[i]);
        extractPathItems(tarr, pp_length_limit, parr);
      }
    }
  }
