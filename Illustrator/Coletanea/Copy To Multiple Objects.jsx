...
if (documents.length>0) {
	if (docRef.selection.length > 1) {
		...
		if (mySelection instanceof Array) {
			...
			//mark stroked Items
			//apply transforms
			var newGroup = mySelection[0].parent.groupItems.add();
			//modify move behavior for changes in JS for CS...
			if (version == "10.0") {
				newGroup.moveToEnd(mySelection[0].parent);
				var tempObj = mySelection[0].duplicate();
				tempObj.moveToEnd(newGroup);
			} else {
				newGroup.move(mySelection[0].parent, ElementPlacement.PLACEATEND);
...
function markStroked(Sel) {
	...
	for (var a=0; a<slen; a++) {
		if (Sel[a].typename == "GroupItem") {
			...
			//
			if(Sel[a].pathItems.length==0){
			//then we don't want it anyway...
			}else{
