//***** FUNÇÃO PARA CONFERIR OVERFLOWS *******************

function overflowTextFrame(documentoAtual, numeroDaPagina){
    
    //alert(numeroDaPagina);
    
        var paginaOverflow = documentoAtual.pages.itemByName(numeroDaPagina+"");
        
            //alert(paginaOverflow.textFrames.);
    
    for(over = 0; over < paginaOverflow.textFrames.length; over++){
        
        //alert(paginaOverflow.textFrames[over].overflows);
        
        if(paginaOverflow.textFrames[over].overflows){
            return true;
        }
    }
            return false;
...
//***** FIM FUNÇÃO PARA CONFERIR OVERFLOWS *******************

//*************** Conta Numero de Graficos das paginas selecionadas *********************

function contaNumeroDeGraficos(documentoAtual, pgInicial, pgFinal){

//var paginaInicial = ""+1;
//var paginaFinal = ""+4;

var paginaInicial = pgInicial;
var paginaFinal = pgFinal;

paginaInicial = documentoAtual.pages.itemByName(paginaInicial);
paginaFinal = documentoAtual.pages.itemByName(paginaFinal);

var arrayPaginas = new Array();
arrayPaginas =  documentoAtual.pages.itemByRange(paginaInicial,paginaFinal);
arrayPaginas = arrayPaginas.name;

var contaNumeroDeGraficos = 0;

for(pagina=0; pagina<arrayPaginas.length; pagina++){

    var paginaAtual = documentoAtual.pages.itemByName(arrayPaginas[pagina]+"");
    
    contaNumeroDeGraficos = contaNumeroDeGraficos +(paginaAtual.allGraphics.length*1);
    
    }

//alert(contaNumeroDeGraficos);

return contaNumeroDeGraficos;

}
//*************** Fim Conta Numero de Graficos das paginas selecionadas *********************
...
function funcaoAjustaLeading(graphics,paragrafo){
        ...
        //Movimenta a equação de acordo com o baseline e a porcentagem
        ...
        myRectangle.anchoredObjectSettings.anchorYoffset = -distanciaBaseline*(porcentagem/100);
...
win.buttons.okBtn.onClick = function()
	{
		var todosOsCampos = ""+win.grupoPrincipal.porcentagem.input.text;
		var todosOsCampos = todosOsCampos.split(",");
		//alert(todosOsCampos.length);
	
		if(todosOsCampos.length>1){					
...
porcentagem = win.grupoPrincipal.porcentagem.input.text*1;
...
var tamanhoDoProcesso = contaNumeroDeGraficos(documentoAtual,win.grupoPrincipal.inicio.lista.selection.text,win.grupoPrincipal.fim.lista.selection.text);
//alert(tamanhoDoProcesso);
...
for(s=0; s<documentoAtual.stories.length; s++){
    ...
    for(p=0; p < atualStory.paragraphs.length; p++){
        ...
        for(g=0; g < paragrafoAtual.allGraphics.length; g++){
            ...
            try{
                numeroDaPagina = paragrafoAtual.allGraphics[g].parentPage.name*1;
                }
            catch(e){
                numeroDaPagina = 0;
                    }
            //alert(numeroDaPagina);
            //alert(win.grupoPrincipal.inicio.lista.selection.text*1);
            
            if(win.grupoPrincipal.inicio.lista.selection.text*1 <= numeroDaPagina && win.grupoPrincipal.fim.lista.selection.text*1 >= numeroDaPagina){ 
                ...
                //documentoAtual.zoom = ZoomOptions.FIT_PAGE;
                app.selection = paragrafoAtual.allGraphics[g].parentPage;
                //alert(graficoNaPagina);
                
                //alert(overflowTextFrame(documentoAtual,numeroDaPagina));
                if(overflowTextFrame(documentoAtual,numeroDaPagina)){
                    throw "Página "+graficoNaPagina+": Alça com texto oculto.";
                    }
...
alert("Concluido com sucesso","Lucas ®",false);
    ...
    if(e == "Error: Invalid object for this request." || e == "TypeError: null is not an object"){
        e = "Página "+graficoNaPagina+": Alça com texto oculto,\nou elementos fora da página.";
        }
